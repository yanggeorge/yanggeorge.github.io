...
&nbsp;&nbsp;&nbsp;&nbsp;拿到这个题，最先想到的还是比较基本的时间复杂度O(N*N)
的解法。那就实现一下吧，最起码可以用进行优化后的比较。
...
&nbsp;&nbsp;&nbsp;&nbsp;对于这样的题，还是尝试了能不能使用动态规划，
但是仔仔细细列出来，发现怎么也找不到递推的方法。只好放弃。不过在思考
...
数组数的个数多起来，就变得复杂了，无法这样简单的求解了。真是非常的泄气啊。
想了好久也没有相出第二可用的思路来。脑子里把曾经用到的方法想了一个遍，
也还是不行。

## 用二维图描述问题 ##

&nbsp;&nbsp;&nbsp;&nbsp;物理学科中经常使用的一个策略就是转换坐标系，可以神奇的
化繁为简。那么我是否可以这样做呢。看看能不能用二维的图来重新展示这个问题。
于是我就画了这样一个图。这是对数组`[5,1,6,7,2,3]`画的表格。横轴表示数组的index，
纵轴表示大小。
...
因为我一直还是老思路，总想着根据第一个数字把剩下的数组分成两批。
后来我想，既然是二维图，**我既可以从左向右，一列一列的看，也可以从上往下一行一行的
看。** 不错，终于有了新的思路。

0. 初始化一个长度为N的smaller数组。
1. 对数组从大到小排序。
2. 取出最大的数，这个数的 `n = smaller_num(Nmax) = 数组的长度 - pos(Nmax)`
3. `smaller[index(Nmax)] = n`
4. 对于index大于该数的所有数字的index-1。
5. 重复2
...
遇到了时间复杂度过高的问题。因为实现步骤3的方法导致整个程序的
时间复杂度是O(N*N)。那么有什么办法能够优化呢。嗯，肯定有办法的。
我们这样来做。

1. 保存一个原始索引的`(origin_index,value)`的`origin`List(tuple)
2. 再保存一个从大到小排序后的`(origin_index,value)`的`l`List。
3. 对`l`从大到小进行遍历：根据orgin_index使用二分查找
...
&nbsp;&nbsp;&nbsp;&nbsp;而相等的数怎么办呢？嗯我们可以在排序
的时候保证序列不但满足值的升序排列，还能保证索引值的升序排列。
这样，最右侧的最大数永远在序列的末尾。这个数的右侧不会有相等的值。
真是很不错啊。
